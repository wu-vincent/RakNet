...
Every time an NAK is received, UDT updates the largest sequence number sent so far (`LastDecSeq`). A congestion period is defined as the time between two NAKs in which the biggest lost packet sequence number carried in the NAK is greater than the `LastDecSeq`.
...
These four parameters are used in rate decrease (initial values in parentheses): `AvgNAKNum` (1), `NAKCount` (1), `DecCount` (0), `LastDecSeq` (initial sequence number - 1).

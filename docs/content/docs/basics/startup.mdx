...
Call `RakPeerInterface::Startup()` first. `Startup()` will:
...
In most cases, you can pass something like:
...
This is for advanced use cases where you bind multiple network cards -- for example, one card for the secure server behind the LAN and another for the internet. To access different bindings, pass the binding index to functions in `RakPeerInterface` that have a `connectionSocketIndex` parameter.
...
For Windows, this is the priority of the RakPeer update thread passed to `_beginthreadex()`. For Linux, it is passed to `pthread_attr_setschedparam()` for `pthread_create()`. The default parameter is `-99999`, which on Windows means priority `0` (`NORMAL_PRIORITY`) and on Linux means priority `1000`. The default is appropriate for Windows. On Linux, set this to your normal priority thread value.

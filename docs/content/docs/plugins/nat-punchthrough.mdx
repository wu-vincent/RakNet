...
NAT (network address translation) is used by routers to map addresses behind the router to a single destination address, using different ports. For example, if two computers share a router with one ISP, both computers use the same IP address but with different source ports than the application assigned. The router keeps a lookup table of mappings, so when a remote computer replies, traffic is routed to the correct local computer behind the NAT.

The problem with NAT is that remote computers cannot initiate sends to local computers because no mapping yet exists. Therefore, if two computers both behind NAT try to connect, neither can succeed. This affects voice communication, peer-to-peer games, and games where users host and the host is behind a NAT. `NatPunchthrough` eliminates the need for users to manually configure port mappings on their routers.

## NAT Punchthrough Overview

The `NatPunchthroughClient.cpp` plugin requires a user-hosted server, not behind NAT, running `NatPunchthroughServer.cpp` that both clients can connect to. The server finds the external IP address of each client and tells both clients to connect to that address at the same time. If that fails, each client attempts to estimate the ports used by the other. If that also fails, the process repeats once more in case later port estimation opened a prior port. If all attempts fail, the plugin returns `ID_NAT_PUNCHTHROUGH_FAILED`.
...
- If your game is client/server only, you can enforce that servers must support UPNP. See _DependentExtensions\\miniupnpc-1.6.20120410_. Most routers support this, and assuming UPNP passes, anyone can connect to you.
</Callout>
...
7.  If at any point a datagram arrives from the remote peer, the system enters state `PUNCHING_FIXED_PORT`. Datagrams are sent only to that IP/port combination for the remainder of the algorithm. If the reply arrives on the remote system, the NAT is considered bidirectional and `ID_NAT_PUNCHTHROUGH_SUCCEEDED` is returned to the user.
8.  When NAT is open, or if all ports are exhausted, P1 and P2 send to F that they are ready for a new punchthrough attempt.
...
**Address-Restricted cone NAT**: Accepts datagrams to a port as long as the datagram source IP address is a system the NAT has already sent to. Accepts the first datagram if both systems send simultaneously. Otherwise, accepts the first datagram after one outbound datagram has been sent.

**Port-Restricted cone NAT**: Same as address-restricted cone NAT, but the sender must target both the correct remote IP address and correct remote port. The same source address and port to a different destination uses the same mapping.
...
Call `OpenNAT` with the `RakNetGUID` (globally unique identifier) of the remote system you want to connect to. To obtain the `RakNetGUID`, either transmit it with your own code on the server, upload it to the `PHPDirectoryServer`, or use a plugin that stores it, such as `LightweightDatabase`: `natPunchthroughClient.OpenNAT(remoteGuid, serverSystemAddress);`. To read your own `RakNetGUID`, use `RakPeerInterface::GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS);`
...
It may take over 10 seconds to try every possible port twice, although it often succeeds within a few seconds. To get text messages on the progress, use `NatPunchthroughClient::SetDebugInterface()`.
...
`ID_NAT_PUNCHTHROUGH_SUCCEEDED` means punchthrough succeeded, and you can now connect or send other messages to the remote system. `Packet::SystemAddress` is the address of the system you can now connect to. Any other `ID_NAT_*` means punchthrough failed. See `MessageIdentifiers.h` for the list of codes and comments on each.
...
With some low-quality or homemade routers, NAT punchthrough will not work. For example, a router that picks a new random port for each outgoing connection and only allows incoming connections to that port will never succeed. This occurs about 5% of the time. To handle this case, RakNet provides the `UDPProxy` system. It uses a server that you run to route messages between the source and destination client transparently. This even works for routing datagrams from games not using RakNet (though you need RakNet to set up the forwarding). The combination of `NATPunchthrough` and `UDPProxy` enables any system to connect to any other system with a 100% success rate, provided you host enough proxy servers to forward all the traffic.
...
-   **`UDPProxyCoordinator`**: Runs on the server that receives all requests from `UDPProxyClient` and all logins from `UDPProxyServer`.
-   **`UDPProxyServer`**: Performs the UDP datagram forwarding via a composite instance of `UDPForwarder.cpp`.
...
Assuming you are connected to the coordinator and the coordinator is running the plugin, your event handler class created in step 2 receives a callback within a second or two. `UDPProxyClientResultHandler::OnForwardingSuccess` is returned if a `UDPProxyServer` has been assigned to forward datagrams from the source system specified in step 6 to the target system specified in step 6. For example, to connect to the remote system: `rakPeer->Connect(proxyIPAddress, proxyPort, 0, 0);`

</Step>
</Steps>

If more than one server is available, and both source and target relay systems are running RakNet, source and target automatically ping all available servers. The servers are attempted in order of lowest ping sum to highest. This is based on the assumption that the lowest ping sum corresponds to the server with the shortest path between the two systems, and therefore the least lag.
...
Log in to the coordinator. This can be done at runtime, so you can dynamically add more forwarding servers as your game grows.

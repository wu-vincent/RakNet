...
<Callout type="warn">
**Do not forget to escape user input!** `RakString::SQLEscape()` can be used to do this. It will append a backslash before any quotes, double quotes, or backslashes that already exist in the string. This system is insecure by default, as basically anyone can execute any query. If you want security, derive from `SQLite3Plugin` to control who can send queries, or put constraints in the database itself.
</Callout>

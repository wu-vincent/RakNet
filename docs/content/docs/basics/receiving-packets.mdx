...
-   We cast the packet data to a pointer to the appropriate type of struct to avoid the copy overhead that would occur if we were to actually create the struct. However, in this case if we change any of the data in the struct it will change the packet as well. This may or may not be what we want. Take care when relaying messages as a server, as this can cause unintended bugs.

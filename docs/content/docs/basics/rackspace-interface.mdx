...
### Programmatically manage application servers

Rackspace provides an HTTPS based API. RakNet supports HTTPS when using the class [TCPInterface](/utilities/tcp-interface) and compiled with OPEN_SSL_CLIENT_SUPPORT 1, defined in RakNetDefines.h.
...
The callback will return HTML event codes 200, 203, 400, 500, 503, 401, 400, or 413. 200 and 203 indicate success. The event code will be returned in the eventType parameter. The actual HTML will be returned in htmlAdditionalInfo. Example output:

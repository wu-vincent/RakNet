...
If you wish to use the DLL\_Swig project you will need to add SWIG to the PATH variable as shown below.
...
<Callout type="info">
This step on Windows can be skipped if you are using the included DLL\_Swig project as it runs the tools on rebuild.
</Callout>
...
A C# sample project is included under `DependentExtensions\Swig\SwigWindowsCSharpSample`. The DLL file is not included and needs to be copied to `bin/Debug` or `bin/Release` depending on whether you are doing a release or debug build. If the DLL\_Swig project is used, the DLL is copied automatically.

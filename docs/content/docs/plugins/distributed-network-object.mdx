...
The distributed network object system is the highest level layer over RakNet. The concept is straightforward: objects that are instantiated on one system are instantiated on all systems. Objects that are destroyed on one system are destroyed on all systems. Tagged memory is matched between systems. When a new player connects, these objects are also created on that player's system.

This maps directly to game objects. For example, one tank in a multiplayer game with twenty players has been instantiated twenty times. However, as far as the player is concerned there is only one tank. The player expects that the position, orientation, number of shells left, and amount of armor is the same on all systems.

Traditionally, to maintain a tank you have to craft a series of custom packets for the tank. One packet to describe position, another for the tank to shoot, and another to have the tank take damage. Using the distributed network object system, you can synchronize the tank, those 3 member variables, and everything matches automatically.

**Strengths**

- Implementation takes minimal effort and once it works it is foolproof.
...
Before using the distributed object system, register your instance of `RakClient` and/or `RakServer` with the `DistributedNetworkObjectManager`.
...
The multiplayer class automatically handles distributed object network messages. If you do not use the multiplayer class, you must handle these packet types yourself:
...
Pass them to the `DistributedNetworkObjectManager` in exactly the same fashion as is done in `Multiplayer.h`.

Once that is done, follow these steps per class:
...
```cpp
REGISTER_X_DISTRIBUTED_OBJECT_MEMBERS(
    BaseClass, SynchronizationMethod, AuthoritativeNetwork, VariableType, VariableName, ...)
```
...
- `DOM_SERVER_AUTHORITATIVE` -- The server is in charge of the variable. Client changes are overwritten when the server next changes it.
...
class Tank : public DistributedNetworkObject {
  ...
  public:
    ...
    REGISTER_2_DISTRIBUTED_OBJECT_MEMBERS(DistributedNetworkObject,
                                          DOM_INTERPOLATE_COMPRESSED,
                                          DOM_CLIENT_AUTHORITATIVE,
                                          float,
                                          turretAngle,
                                          DOM_COPY_UNCOMPRESSED,
                                          DOM_SERVER_AUTHORITATIVE,
                                          Vector,
                                          position)
};

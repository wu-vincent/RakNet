...
1. Add `#include "DistributedNetworkObjectHeader.h"` to the header file for your class.
2. The basemost class should derive from `DistributedNetworkObject`.
3. If you plan to instantiate this class, add `REGISTER_DISTRIBUTED_CLASS(ClassName)` in `main()` or `WinMain()`, with the name of your class replacing `ClassName`.
4. Once per game cycle call `objectInstance->UpdateDistributedObject(ClassName)` where `objectInstance` is the instance of your class and `ClassName` matches the identifier passed to `REGISTER_DISTRIBUTED_CLASS`.

...
## Team and member management for client/server and peer-to-peer
...
Have those game classes contain a corresponding `TM_Team` or `TM_TeamMember` instance. Operations on teams are performed by those instances. Use `SetOwner()` to refer to the parent object when using composition.
...
Call `AddWorld()` to instantiate a `TM_World` object that contains references to your `TM_TeamMember` and `TM_Team` instances.
...
`DeserializeConstruction()` must be called on `TM_Team` before `DeserializeConstruction()` is called on `TM_TeamMember` that is a member of that team. If using `ReplicaManager3`, call `ReplicaManager3::Reference()` on `TM_TeamMember` later than any `ReplicaManager3::Reference()` calls to `TM_Team`.

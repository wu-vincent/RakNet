...
The basic class has functions to connect to the database and to create and destroy the required tables. The sample at `Samples/LobbyServerTest/LobbyServerTest.cpp`, in the project `LobbyServer`, demonstrates this.

One additional requirement is the use of the `Functor` class. A functor is a class in RakNet that implements a specific unit of functionality that runs asynchronously in a `FunctionThread`, another utility class that is part of RakNet. The key functor to be aware of is the one that adds new titles (games/applications) to the database.
...
See `LobbyServerTest` for a console application that runs the `LobbyServer`.
...
-   All source files in `Samples\LobbyServerTest`, if you want to use the default console application.

<Callout type="warn">
**Dependencies:** PostgreSQL 8.2 or newer is required, installed at C:\\Program Files\\PostgreSQL\\8.2. Change the project property paths if your installation directory differs. Ensure you check development tools in the PostgreSQL installer, or the headers and libs will not be installed.
</Callout>
...
Call `SetCallbackInterface()` with a derived implementation of `LobbyClientInterfaceCB`. All calls are asynchronous, so each call returns its results (success or failure) to your registered callback.
...
Call `SetTitleLoginId()` with information identifying what title you are playing (can be done later if multiple games are allowed per lobby). This value is hardcoded into the application, returned when the title was added to the server (as documented above).
...
Call `Login()` with the account you created (or previously stored). Wait for `LobbyClientInterfaceCB::Login_Result()` to indicate success. If you have friends, they receive `LobbyClientInterfaceCB::FriendStatus_Notify()` calls indicating you are online.
...
Once the game starts, you receive `LobbyClientInterfaceCB::StartGame_Notify()`. This provides the IP addresses of all players, participants, who the moderator is, user handles, and other information relevant to playing the game. At this point you can disconnect from the lobby. If you do not, you are automatically sent back to the main lobby (outside of any room).
...
-   All source files in `Samples\LobbyClientTest`, if you want to use the default console application as a framework for your own GUI implementation.

...
The `NetworkIDObject` class is an optional base class that automatically assigns identification numbers (`NetworkID`) to objects that inherit from it. This is useful for multiplayer games, because otherwise you have no way to refer to dynamically allocated objects on remote systems.
...
Assigns a `NetworkID` to the object. A typical use case: the server creates a new game object and sends that object's data to the clients. The client creates a class of the same type, reads the `NetworkID` encoded in the user message, and calls `SetNetworkID` on the same object.
...
The following example stores a pointer to a class, retrieves it back, and asserts that the system worked:
...
The following example creates an object on a remote computer, with the same NetworkID assigned to both:

...
The `BitStream` class, under the namespace `RakNet`, wraps a dynamic array for packing and unpacking bits. Its four main benefits are:
...
With structs you have to predefine your structures and cast them to a `(char*)`. With a bitstream, you can choose to write blocks at runtime, depending on the context. Bitstreams can compress the native types using `SerializeBitsFromIntegerRange` and `SerializeFloat16()`.

You can also write individual bits. When writing booleans, BitStream automatically writes only one bit. This is also useful for encryption since your data is no longer byte-aligned.

## Writing Data

`BitStream` is templated to accept any type of data. For built-in types such as `NetworkIDObject`, it uses partial template specialization to write the type more efficiently. For native types or structures, it writes the individual bits, similar to `memcpy`. You can pass structs containing multiple data members to bitstreams. However, you may want to serialize each individual element for correct [endian](http://en.wikipedia.org/wiki/Endianness) swapping (needed for communication between PCs and Macs, for example).
...
One constructor overload takes a length in bytes as a parameter. If you know the approximate size of your data, pass this value when creating the bitstream to avoid internal reallocations.

See [Creating Packets](/basics/creating-packets) for more details.

## Reading Data

Reading data is straightforward. Create a bitstream and assign your data in the constructor.
...
// With endian swapping (__BITSTREAM_NATIVE_END should just be commented in
// RakNetDefines.h)
#undef __BITSTREAM_NATIVE_END
#include "BitStream.h"
...
You can use the same function for both reading and writing by using `BitStream::Serialize()` instead of `Read()` or `Write()`.
